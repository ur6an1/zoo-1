...
PLAN_AI_LIMITS: dict[str, int | None] = {
    PLAN_FREE: _settings.FREE_AI_LIMIT,
    PLAN_BASIC: None,
    PLAN_PRO: None,
}

PLAN_PET_LIMITS: dict[str, int | None] = {
    PLAN_FREE: _settings.FREE_PET_LIMIT,
    PLAN_BASIC: 5,
    PLAN_PRO: None,
}
...
def _ai_limit_for_plan(plan_tier: str) -> int | None:
    return PLAN_AI_LIMITS.get(plan_tier, _settings.FREE_AI_LIMIT)


def _pet_limit_for_plan(plan_tier: str) -> int | None:
    return PLAN_PET_LIMITS.get(plan_tier, _settings.FREE_PET_LIMIT)
...
def format_subscription_info(settings: UserSettings) -> str:
    ...
    remaining = max(_settings.FREE_AI_LIMIT - used, 0)

    return (
        "📋 <b>Текущий тариф: Бесплатный</b>\n\n"
        f"🤖 AI-запросы: {remaining}/{_settings.FREE_AI_LIMIT} в день\n"
        f"🐾 Питомцы: до {_settings.FREE_PET_LIMIT}\n"
        "❌ PDF-экспорт\n"
        "❌ Погодные уведомления\n"
        "❌ Голосовые заметки\n\n"
