...
async def pay_card(callback: CallbackQuery):
    ...
    try:
        ...
        if get_settings().RECEIPT_EMAIL:
            payment_request["receipt"] = {
                "customer": {"email": get_settings().RECEIPT_EMAIL},
                "items": [
                    {
...
async def pay_stars(callback: CallbackQuery, bot: Bot):
    ...
    description = "Zoo Bot: тариф Базовый на 30 дней" if plan_key == "basic" else "Zoo Bot: тариф PRO на 30 дней"
    try:
